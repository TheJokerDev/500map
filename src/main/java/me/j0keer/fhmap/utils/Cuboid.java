...
public class Cuboid {
    ...
    public List<SPBlock> blockList() {
        final List<SPBlock> bL = new ArrayList<>(this.getTotalBlockSize());
        for(int x = this.xMin; x <= this.xMax; ++x) {
            for(int y = this.yMin; y <= this.yMax; ++y) {
                for(int z = this.zMin; z <= this.zMax; ++z) {
                    final Block b = this.world.getBlockAt(x, y, z);
                    bL.add(new SPBlock(b));
                }
            }
        }
        return bL;
    ...
    public List<SPBlock> getWallBlocks() {
        List<SPBlock> result = Lists.newArrayList();
        for (int x = xMin; x <= xMax; x++) {
            for (int y = yMin; y <= yMax; y++) {
                for (int z = zMin; z <= zMax; z++) {
                    ...
                    if (components >= 2) {
                        result.add(new SPBlock(world.getBlockAt(x, y, z)));
                    }
                }
            }
        }
